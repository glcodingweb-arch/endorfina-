...
import { ArrowLeft, CheckCircle, Clock, XCircle, AlertTriangle, Loader2 } from 'lucide-react';
...
type DeliveryStatus = 'Pendente' | 'Entregue' | 'NaoAtendido' | 'Problema';
    ...
    return (
        <>
            <div className="space-y-8">
                <header>
                    <Button variant="ghost" onClick={() => router.push('/delivery')} className="mb-4 pl-0">
                ...
                <Card>
                    <CardHeader>
                        ...
                        <div className="flex flex-wrap gap-2 pt-2">
                            {(['all', 'Pendente', 'Entregue', 'NaoAtendido', 'Problema'] as const).map(f => (
            ...
            <Dialog open={!!selectedOrder} onOpenChange={(isOpen) => { if(!isOpen) { setSelectedOrder(null); form.reset(); }}}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Pedido <span className="font-mono">{selectedOrder?.orderNumber}</span> para <strong className="font-bold">{selectedOrder?.responsibleName}</strong>
                            <br/>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(handleUpdateStatus)} className="space-y-4">
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <RadioGroup onValueChange={field.onChange} className="space-y-2">
                                                {Object.entries(statusConfig).filter(([key]) => key !== 'Pendente').map(([key, config]) => (
                            ...
                            />
                            <DialogFooter>
                                <Button type="button" variant="ghost" onClick={() => setSelectedOrder(null)}>Cancelar</Button>
