...
export default function ManageRacePage() {
  ...
  const { data: raceData, loading } = useDoc<Race>(raceRef);
  
  const tagsQuery = useMemoFirebase(() => firestore ? collection(firestore, 'raceTags') : null, [firestore]);
  const { data: tagsData, loading: loadingTags } = useCollection<RaceTag>(tagsQuery);
  ...
  useEffect(() => {
    if (raceData) {
      form.reset({
        ...raceData,
        kitItems: raceData.kitItems || [],
        showKitItems: raceData.showKitItems ?? true,
        date: raceData.date.split('T')[0], // Format date for input
        options: raceData.options.map(opt => ({
          ...opt,
          bibPrefix: opt.bibPrefix ?? null,
          lots: opt.lots.map(lot => ({
            ...lot,
            startDate: lot.startDate?.split('T')[0] ?? '',
            endDate: lot.endDate?.split('T')[0] ?? '',
          }))
        })),
        tags: raceData.tags || [],
        kitDelivery: raceData.kitDelivery || { enabled: false, price: 25 },
        kitPickup: raceData.kitPickup || { enabled: true, location: '', details: '' },
      });
    }
  }, [raceData, form]);
  ...
  async function onSubmit(values: RaceFormValues) {
    if (!firestore || !raceId) return;
    
    const raceDoc = doc(firestore, 'races', raceId);
    ...
    try {
        ...
        toast({
            ...
            description: 'Ocorreu um problema ao tentar salvar as alterações.',
        });
    }
  }
    ...
    return (
        <div className="text-center">
            <h1 className="text-2xl font-bold">Corrida não encontrada.</h1>
             <Button onClick={() => router.push('/admin/events')} className="mt-4">Voltar para Eventos</Button>
        </div>
    )
  }
      ...
      <div className="mb-8">
        <h1 className="text-4xl font-extrabold tracking-tight">Gerenciar: {raceData.name}</h1>
         <p className="mt-2 text-lg text-muted-foreground">
          Edite os detalhes do evento e gerencie as inscrições.
        </p>
      </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              ...
              <Button
                type="button"
                variant="outline"
                onClick={() => append({ distance: '', bibPrefix: null, lots: [{ name: '1º Lote', price: 0, startDate: '', endDate: '' }] })}
              >
